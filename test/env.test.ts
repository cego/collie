...
test("plugin env is read from the herdr-provided variables", () => {
  const env = readEnv({
    ...
    HERDR_PLUGIN_ROOT: "/plugins/cego.workflows",
  ...
  expect(env.pluginRoot).toBe("/plugins/cego.workflows");
...
test("cwd and ids fall back to the invocation context", () => {
  ...
  expect(env.configDir).toBe("/home/x/.config/herdr/plugins/cego.workflows");
});
